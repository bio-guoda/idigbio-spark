...
class OccurrenceSelectors$Test extends FlatSpec with Matchers {
  ...
  "a taxon filter" should "always match when empty" in {
    val selectors = apply(OccurrenceSelector("", "ENVELOPE(4,5,52,50)", ""))
    val matching = Occurrence("51", "4.1", "Insecta|Aedes mitchellae", "2015-01-01", "someId", "20140101", "someSource")
    selectors(matching) should be(true)
  }
  ...
  // https://github.com/gimmefreshdata/freshdata/issues/55
  "a taxon filter" should "filter taxa by canonical name, ignoring author" in {
    val selectors = apply(OccurrenceSelector("Aedes mitchellae", "ENVELOPE(4,5,52,50)", ""))
    val matching = Occurrence("51", "4.1", "Insecta|aedes mitchellae (duck 2015)", "2015-01-01", "someId", "20140101", "someSource")
    selectors(matching) should be(true)
  }
